...
class MyMadImageComponent extends Component {
	...
	*/
	...
	function cropImage($src, $options = array()) {		
	...
	function makeThumb($src, $options = array()) {
		switch (".".pathinfo($src, PATHINFO_EXTENSION)) {
			case '.jpg':
			case '.jpeg':
				$img = imagecreatefromjpeg($src);
		...
		$x_ratio = $options['dw'] / $options['sw'];
		$width = floor($options['sw'] * $x_ratio);
		$height = floor($options['sh'] * $x_ratio);
